...
const CardForm = ({ boardId, onSuccess, onClose }) => {
  ...
  // const apiKey = oJtL3eVFvCKSgxVmUkFb8SjqM27vGVuS;
  ...
  const createNewCard = async () => {
    try {
      ...
      const response = await axios.post(
        `https://kudosboard-slb3.onrender.com/boards/${boardId}/cards`,
        {
          title,
          description,
          gif,
  ...
  return (
    <div className="overlay">
      <div className="new-card-form">
        ...
        <input type="text" placeholder="Enter card title" value={title} onChange={(e) => setTitle(e.target.value)} required/>
        ...
        <button className="search-button" type="search" onClick={searchGifs}>Search</button>
        {gifOptions.length > 0 && (
